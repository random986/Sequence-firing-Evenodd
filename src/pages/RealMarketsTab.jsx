...
export default function RealMarketsTab() {
  ...
  const activeMarketSym = selectedMarket === 'AUTO' ? ALL_MARKETS[0] : selectedMarket;
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 12, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginRight: 8, paddingRight: 8, borderRight: '1px solid var(--border)' }}>
              <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Auto Select:</span>
              <button 
                  onClick={() => setSelectedMarket(selectedMarket === 'AUTO' ? ALL_MARKETS[0] : 'AUTO')}
                  style={{
                    width: 36, height: 20, borderRadius: 10,
                    background: selectedMarket === 'AUTO' ? 'var(--success)' : 'var(--border)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginRight: 8, paddingRight: 8, borderRight: '1px solid var(--border)' }}>
               ...
               >
                 <option value="AUTO">AUTO (Hottest)</option>
      ...
      <div className="glass" style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 16px', fontFamily: 'var(--font-mono, ui-monospace, monospace)', fontSize: 11 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, width: '100%', minWidth: 0 }}>
          <span style={{ color: 'var(--text-muted)', letterSpacing: '0.1px', lineHeight: 1.5 }}>
            ...
            {' · '}Selected: <strong style={{ color: 'var(--text-primary)' }}>{selectedMarket === 'AUTO' ? `AUTO (${MARKET_LABELS[activeMarketSym] || activeMarketSym})` : (MARKET_LABELS[activeMarketSym] || activeMarketSym)}</strong>
          ...
          <div className="glass" style={{ padding: '16px 20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(110px, 1fr))', gap: 8 }}>
              {Object.values(markets).filter(m => !m.isClosed).sort((a, b) => (b.mcs?.total ?? 0) - (a.mcs?.total ?? 0)).slice(0, 10).map((m, idx) => (
                <div key={m.symbol} style={{ padding: 8, borderRadius: 6, background: 'var(--bg-primary)', border: `1px solid ${idx === 0 && selectedMarket === 'AUTO' ? 'var(--amber)' : 'var(--border)'}` }}>

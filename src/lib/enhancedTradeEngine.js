...
const POST_TRADE_TICK_COOLDOWN = 2;
...
const TOURNAMENT_STRATEGIES = new Set(['BOTH5', 'BOTH', 'OMNISNIPER']);
...
class EnhancedTradeEngine {
  ...
  _findBestMarketForDirection(targetDir, opts = {}) {
    const isOu = targetDir === 'OVER5' || targetDir === 'UNDER5'
      || targetDir.startsWith('OVER') || targetDir.startsWith('UNDER');
    const ch = this.channels[targetDir];
    const legLosses = ch?.consecutiveLosses || 0;
    const minPct = targetDir === 'OVER5'
      ? (legLosses >= 2 ? 40 : 42)
      : (targetDir === 'UNDER5' ? 48 : 48);
    const altCap = targetDir === 'OVER5' ? 0.65 : 0.68;
    let best = null;

    for (const market of MARKETS) {
      if (opts.excludeMarket && market === opts.excludeMarket && opts.skipExclude !== false) continue;
      if (this.marketStats[market]?.quarantinedUntil > Date.now()) continue;
      if (this._isLegMarketBlocked(targetDir, market)) continue;
      ...
      if (isOu && d5Pct >= (targetDir === 'OVER5' ? 11 : 13)) continue;

      const bias = this._getDistributionBiasPct(targetDir, scores);
      if (bias < minPct) continue;
      if (this._computeAlternationRate(ticks, targetDir) > altCap) continue;

      const streaks = this._calcDigitStreaks(ticks);
      const lossStreak = targetDir === 'OVER5' ? streaks.overLossStreak
        : targetDir === 'UNDER5' ? streaks.underLossStreak
        : targetDir === 'EVEN' ? streaks.evenLossStreak
        : targetDir === 'ODD' ? streaks.oddLossStreak
        : 0;

      const revProb = this._computeLunarReversalProb(ticks, targetDir, 2);
      const baseWR = this._getBaselineWinRate(targetDir);
      const revEdge = (revProb ?? baseWR) - baseWR;
      const chopIndex = this._calcChopIndex(ticks);
      const winChance = this._estimateTournamentWinChance(market, targetDir, lossStreak, ticks, scores);

      const mStats = this.marketStats[market] || {};
      const mTotal = (mStats.totalSessionWins || 0) + (mStats.totalSessionLosses || 0);
      const marketBonus = mTotal >= 2 ? ((mStats.totalSessionWins || 0) / mTotal - 0.45) * 20 : 0;
      const legMarketPenalty = this._getLegMarketLossStreak(targetDir, market) * 30;
      const onActivePenalty = market === this.activeMarket && legLosses >= 2 ? 20 : 0;

      let score = bias * 2.5 + revEdge * 35 + lossStreak * 5 + winChance * 1.2
        + chopIndex * 0.06 + marketBonus + legLosses * 8 - legMarketPenalty - onActivePenalty;
      if (isOu) score -= d5Pct * (targetDir === 'OVER5' ? 4 : 2.5);
      ...
      if (!best || score > best.score) {
        best = {
          ...
          winChance,
          lossStreak,
          detail: `${targetDir} ${bias.toFixed(0)}% VL${lossStreak} D5${d5Pct.toFixed(1)}% sc${score.toFixed(0)}`,
        };
      }
    }
    return best;
  ...
  _refreshLegBestMarkets() {
    ...
    const now = Date.now();
    if (now - (this._lastLegBestRefreshAt || 0) < 120) return;
    this._lastLegBestRefreshAt = now;
    for (const dir of this._getStrategyDirs()) {
      const best = this._findBestMarketForDirection(dir);
      if (best) this._legBestMarket[dir] = { ...best, at: now };
    }
  }

  _maybeRotateMarketForLeg(dir, reason = '') {
    if (this.config?.autoSwitchMarkets === false || !dir) return false;
    const ch = this.channels[dir];
    const legLosses = ch?.consecutiveLosses || 0;
    const marketLosses = this._getLegMarketLossStreak(dir, this.activeMarket);
    const threshold = this._getLegSwitchThreshold(dir);
    const shouldRotate = legLosses >= threshold || marketLosses >= 2
      || (dir === 'OVER5' && legLosses >= 2);

    if (!shouldRotate) return false;

    const pick = this._findBestMarketForDirection(dir, { excludeMarket: this.activeMarket })
      || this._findBestMarketForDirection(dir)
      || this._legBestMarket[dir];

    if (!pick?.market) {
      this.sendLog(`⚠️ [${dir}] ${legLosses} leg / ${marketLosses} market losses — no alternate market`);
      return false;
    }
    if (pick.market === this.activeMarket && marketLosses < 2) return false;

    const old = this.activeMarket;
    this.activeMarket = pick.market;
    if (this.onMarketSwitch) this.onMarketSwitch(this.activeMarket);
    this.nextAllowedTradeTime = 0;
    this.sendLog(
      `🌍 [${dir}] leg rotation · ${MARKET_LABELS[old] || old || '?'} → ${MARKET_LABELS[pick.market]} · ` +
      `${pick.detail}${reason ? ` · ${reason}` : ''} · leg ${legLosses}L · mkt ${marketLosses}L`
    );
    return true;
  }
  ...
  start(config) {
    ...
    if (this.strategy === 'MATCHES') {
      ...
      this.config.recoveryEnabled = true;
      const pick = this._scanWinningPairMarket(this.strategy === 'OU_WINNING');
      this.activeMarket = pick?.market || scanner.getBest(this.strategy === 'OU_WINNING' ? 'BOTH5' : 'BOTH');
    } else {
    ...
    if (this._usesTournamentMode()) {
      ...
      const modeLabel = this.strategy === 'BOTH5' ? 'OVER or UNDER' : 'EVEN or ODD';
      this._logEnsembleAlgorithms();
      this.sendLog(
        `🏆 Matrix Sniper · 15×2 sides · S1 exhaust S2 boundary S3 cluster · single-flight · ×${this.config.martMultiplier || 2}`
      );
      this._updateTournamentStatus();
    } else if (this.strategy === 'OMNISNIPER') {
      // OMNISNIPER now uses the exact same tournament architecture as BOTH/BOTH5
      this._sessionMartingaleStep = 0;
      this._openTournamentContracts = new Map();
      this._settledContractIds = new Set();
      this._pendingTournamentBuy = false;
      ...
      this._logEnsembleAlgorithms();
      this.sendLog(
        `🎯 Omnisniper · 15×multi-dir · O3 O4 O5 U6 U7 U8 · tournament mode · ×${this.config.martMultiplier || 2}`
      );
      this._updateTournamentStatus();
    } else if (this.strategy === 'OU_WINNING' || this.strategy === 'EO_WINNING') {
      this.config.recoveryEnabled = true;
      const legs = this.strategy === 'OU_WINNING' ? 'OVER5+UNDER5' : 'EVEN+ODD';
      this._initMarketVLState();
      this._startLiveBoardSync();
      this.sendLog(
  ...
  _bindTournamentScanner() {
    this._prevScannerOnUpdate = scanner.onUpdate;
    const onTickSym = (sym) => {
      if (!this.running) return;
    ...
    scanner.onUpdate = (sym, allScores) => {
      if (this._prevScannerOnUpdate) this._prevScannerOnUpdate(sym, allScores);
      onTickSym(sym);
    };
  ...
  _stopTournamentScanner() {
    ...
    if (this._prevScannerOnUpdate != null) {
      scanner.onUpdate = this._prevScannerOnUpdate;
      this._prevScannerOnUpdate = null;
    }
  }
  ...
  _getStrategyDirs() {
    ...
    if (this.strategy === 'BOTH5' || this.strategy === 'OU_WINNING') return ['OVER5', 'UNDER5'];
    return ['EVEN', 'ODD'];
  }
  ...
  _tryFireMartingaleRecoveryTrade(freeSlot) {
    ...
    if (plan.mode === 'dual_hedge') {
      const dirs = this.strategy === 'BOTH5' || this.strategy === 'OU_WINNING'
  ...
  _onSlotSettled(slot, contract) {
    ...
    if (this._isWinningDualStrategy()) {
      ...
      const baseCooldown = this.config.cooldownMs ?? 400;
  ...
  _tryFireDualLossRecovery(isOverUnder) {
    ...
    const pairBest = this._scanWinningPairMarket(isOverUnder);
    if (pairBest?.market) {
      this.sendLog(
        `🛡️ Dual analysis recovery → ${MARKET_LABELS[pairBest.market]} (${dirs.join('+')}) · ${pairBest.detail}`
      );
      void this._fireWinningDualPair(pairBest.market, dirs);
      return;
    }

    const { best } = this._omniScanBestDualMarket(isOverUnder, 'entry');
    let chosen = best;
    if (!chosen) {
      const vlPeek = this._omniScanBestDualMarket(isOverUnder, 'vl');
      chosen = vlPeek.best;
      if (chosen) {
        this.sendLog(
          `🛡️ Recovery fallback → ${MARKET_LABELS[chosen.market]} (${dirs.join('+')}) · ` +
          `${chosen.leg1Label} ${chosen.leg1Streak}/${chosen.req1}, ${chosen.leg2Label} ${chosen.leg2Streak}/${chosen.req2}`
        );
      }
      const leg1 = isOverUnder ? 'OVER5' : 'EVEN';
      const leg2 = isOverUnder ? 'UNDER5' : 'ODD';
      const hint = vlPeek.best ? ` · VL ${MARKET_LABELS[vlPeek.best.market]}` : '';
      if (!chosen) {
        this.updateStatus(`🛡️ Dual hedge waiting — pick market${hint}`, true);
        return;
      }
    }

    this.sendLog(
      `🛡️ Loss recovery dual → ${MARKET_LABELS[chosen.market]} (${dirs.join('+')}) · edge ${chosen.lunarScore.toFixed(1)}`
    );
    void this._fireWinningDualPair(chosen.market, dirs);
  }
  ...
  _stealthBackgroundActivity() {
    ...
    const actions = [
      ...
      () => derivWS.send({ proposal_open_contract: 1, contract_id: 1, subscribe: 0 }).catch(() => {}),
    ];
  ...
  _switchWinningSingleMarket(opts = {}) {
    if (this.config?.autoSwitchMarkets === false) {
      this.sendLog(`ℹ️ Auto-switch disabled. Staying on ${MARKET_LABELS[this.activeMarket] || this.activeMarket}`);
      return false;
    }

    const isOu = this.strategy === 'OU_WINNING';
    const best = this._scanWinningPairMarket(isOu);
    if (!best) {
      this.sendLog('🔄 Winning switch: no cleaner market found — keeping current');
      this.nextAllowedTradeTime = 0;
      return false;
    }
    ...
    const inRecovery = this._getDualRoundLossStreak() > 0 || best.rescueDir;
    const base = this._resolveStake(this.config.baseStake);
    for (const dir of best.dirs) {
      if (!this.channels[dir]) continue;
      if (opts.resetMartingale) {
        this.channels[dir].step = 0;
        this.channels[dir].stake = base;
      } else if (!inRecovery) {
        this.channels[dir].consecutiveLosses = 0;
      }
    }
    if (opts.resetMartingale) {
      this._dualPairStep = 0;
    ...
    this.nextAllowedTradeTime = 0;
    const tag = best.leaderboardRecovery ? ' · leaderboard' : '';
    this.sendLog(`🔄 Winning switch → ${MARKET_LABELS[best.market]} (${best.detail})${tag} · instant paired fire ready`);
    return true;
  }
  ...
  _executeWinningSingleCycle() {
    ...
    const rescueDir = this._getDualRecoveryPriorityDir(dirs);
    const cached = rescueDir && this._legBestMarket[rescueDir];
    if (cached?.market && (this.channels[rescueDir]?.consecutiveLosses || 0) >= 2
      && cached.market !== this.activeMarket
      && !this._isLegMarketBlocked(rescueDir, cached.market)) {
      this.activeMarket = cached.market;
      if (this.onMarketSwitch) this.onMarketSwitch(this.activeMarket);
    }
    ...
    const best = this._scanWinningPairMarket(isOu);
    if (!best) {
      this.updateStatus(isOu ? 'Scan 15 · O/U pair…' : 'Scan 15 · E/O pair…', true);
      this._scheduleNext(OMNI_SCAN_MS);
      return;
    }

    if (!this._passesWinningPairQualityGate(best, isOu)) {
      const rescueDir = best.rescueDir || this._getDualRecoveryPriorityDir(dirs);
      const alt = rescueDir && this._findBestMarketForDirection(rescueDir, { excludeMarket: best.market });
      if (alt?.market) {
        best.market = alt.market;
        best.score = alt.score;
        best.detail = alt.detail;
        if (!this._passesWinningPairQualityGate(best, isOu)) {
          this.updateStatus(`Waiting · ${rescueDir || 'recovery'} needs better market…`, true);
          this._scheduleNext(OMNI_SCAN_MS);
          return;
        }
      } else {
        this.updateStatus(`Waiting · ${best.rescueDir || 'recovery'} needs better market…`, true);
        this._scheduleNext(OMNI_SCAN_MS);
        return;
      }
    }

    if (best.market !== this.activeMarket) {
      this.activeMarket = best.market;
      if (this.onMarketSwitch) this.onMarketSwitch(this.activeMarket);
    }

    this.updateStatus(`Winning pair · ${best.detail}`, true);
    this.sendLog(
      `⚡ Winning pair (same tick) → ${MARKET_LABELS[best.market]} · ${dirs.join('+')} · ${best.detail}`
    );
    this._recordTradeFired();
    this._recordTradeFired();
    void this._fireWinningDualPair(best.market, dirs).then((fired) => {
  ...
  _switchWinningDualMarketByVL() {
    if (this.strategy === 'OU_WINNING' || this.strategy === 'EO_WINNING') {
      return this._migrateWinningDualToBalancedMarket();
    }
    ...
    const isOu = this.strategy === 'OU_WINNING';
    const { best, leg1Label, leg2Label } = this._omniScanBestDualMarket(isOu, 'vl');
    if (!best) {
      this.sendLog('🔄 VL switch: no market with both legs saturated — keeping current');
      return;
    }
    if (best.market === this.activeMarket) {
      this.sendLog(
        `🔄 VL: ${MARKET_LABELS[best.market]} still best (${leg1Label} ${best.leg1Streak}/${best.req1}, ` +
        `${leg2Label} ${best.leg2Streak}/${best.req2})`
      );
      return;
    }
    this.activeMarket = best.market;
    if (this.onMarketSwitch) this.onMarketSwitch(this.activeMarket);
    this.sendLog(
      `🔄 VL switch → ${MARKET_LABELS[best.market]} (${leg1Label} ${best.leg1Streak}/${best.req1} · ` +
      `${leg2Label} ${best.leg2Streak}/${best.req2} · score ${best.lunarScore.toFixed(1)})`
    );
  }
  ...
  _migrateWinningDualToBalancedMarket() {
    ...
    // Determine which leg is under pressure (most consecutive losses)
    const ch1 = this.channels[dirs[0]];
    const ch2 = this.channels[dirs[1]];
    const ch1Losses = ch1?.consecutiveLosses || 0;
    const ch2Losses = ch2?.consecutiveLosses || 0;
    const pressureDir = this._getDualRecoveryPriorityDir(dirs) || (ch1Losses >= ch2Losses ? dirs[0] : dirs[1]);
    const pressureLosses = Math.max(ch1Losses, ch2Losses);

    if (pressureLosses >= 2) {
      const dirBest = this._findBestMarketForDirection(pressureDir);
      if (dirBest?.market && dirBest.market !== currentMarket) {
        this.activeMarket = dirBest.market;
        if (this.onMarketSwitch) this.onMarketSwitch(this.activeMarket);
        this.nextAllowedTradeTime = 0;
        this.sendLog(
          `🌍 DEBT MIGRATION · ${MARKET_LABELS[currentMarket] || currentMarket} → ${MARKET_LABELS[dirBest.market]} · ` +
          `${dirBest.detail} · ${pressureDir} pressure ${pressureLosses} losses`
        );
        return true;
      }
    }
    ...
    for (const market of MARKETS) {
      ...
      // ── Compute alternation rates for BOTH legs simultaneously ──
      const alt1 = this._computeAlternationRate(ticks, dirs[0]);
      const alt2 = this._computeAlternationRate(ticks, dirs[1]);
      // Reject markets where either leg is hyper-alternating (too choppy)
      if (alt1 > 0.72 || alt2 > 0.72) continue;

      // ── Compute reversion probability for the pressure leg ──
      const pressureStreak = pressureDir === dirs[0]
        ? (isOu ? streaks.overLossStreak : streaks.evenLossStreak)
        : (isOu ? streaks.underLossStreak : streaks.oddLossStreak);

      // ── Real-time distribution bias for BOTH legs ──
      const bias1 = this._getDistributionBiasPct(dirs[0], scores);
      const bias2 = this._getDistributionBiasPct(dirs[1], scores);
      const balanceGap = Math.abs(bias1 - bias2);
      const minBias = Math.min(bias1, bias2);

      // ── Reversal probability for the losing direction ──
      const required = 2;
      const revProb = this._computeLunarReversalProb(ticks, pressureDir, required);
      const baseWR = this._getBaselineWinRate(pressureDir);
      const revEdge = (revProb ?? baseWR) - baseWR;
      ...
      const mLosses = mStats.totalSessionLosses || 0;
      const mTotal = mWins + mLosses;
      const marketBonus = mTotal >= 2 ? ((mWins / mTotal) - 0.45) * 15 : 0;

      // ── Compute migration score: favor balanced, non-choppy, high-reversion markets ──
      let score = 0;
      score += minBias * 2.2;               // Reward markets where both sides are healthy
      score -= balanceGap * 1.5;            // Penalize heavy one-sided bias
      score += chopIndex * 0.06;            // Low chop = more predictable flow
      score += revEdge * 30;                // Huge reward for high reversal probability
      score += pressureStreak * 3.5;        // Favor markets that already show the pressure side recovering
      score += marketBonus;                 // Historical edge
      if (isOu) score -= d5Pct * 3.5;       // Penalize digit-5 concentration for O/U

      // Penalize current market so we actually move when stuck
      if (market === currentMarket && pressureLosses >= 2) {
        score -= pressureLosses * 10;
      }

      const detail = isOu
        ? `O${bias1.toFixed(0)}% U${bias2.toFixed(0)}% D5${d5Pct.toFixed(1)}% chop${chopIndex.toFixed(0)} rev${(revProb ?? 0).toFixed(0)}%`
        : `E${bias1.toFixed(0)}% O${bias2.toFixed(0)}% chop${chopIndex.toFixed(0)} rev${(revProb ?? 0).toFixed(0)}%`;

      if (!best || score > best.score) {
        best = { market, score, detail, pressureDir, pressureStreak, dirs };
      }
    }

    if (!best) {
      this.sendLog('🔄 Migration scan: no suitable market found — keeping current');
      return false;
    }

    const oldMarket = this.activeMarket;
    this.activeMarket = best.market;
    if (this.onMarketSwitch) this.onMarketSwitch(this.activeMarket);
    this.nextAllowedTradeTime = 0;

    // Keep martingale state intact — only move the pair, don't reset steps/stakes
    if (best.market !== oldMarket) {
      this.sendLog(
        `🌍 DEBT MIGRATION · ${MARKET_LABELS[oldMarket] || oldMarket} → ${MARKET_LABELS[best.market]} · ` +
        `${best.detail} · ${pressureDir} pressure ${pressureLosses} losses · step ${this._dualPairStep || 0} · ` +
        `seq net $${(this._dualPairNetProfit || 0).toFixed(2)}`
      );
    } else {
      this.sendLog(
        `🔄 Migration scan: ${MARKET_LABELS[best.market]} still best · ${best.detail} · staying`
      );
    }
    return true;
  }
  ...
  async _placeTrade(channelKey, direction, stake, dynamicBarrier = null, marketSymbol = null, opts = {}) {
    ...
    if (!isStreamFresh(tradeMarket, scanner, isFastPass ? 800 : 700)) {
    ...
    try {
      let propRes;
      const buffered = consumeBufferedProposal(
        tradeMarket,
        spec.contract_type,
        cleanStake,
        barrierVal
  ...
  _handleContractUpdate(msg) {
    ...
    const baseCooldown = this.config.cooldownMs ?? 400;
    // Unified cooldown — same speed for wins and losses, no loss-streak escalation
    let cooldownMs = Math.max(80, baseCooldown * 0.2);
    if (this.strategy === 'MATCHES') {
      cooldownMs = 400;
    }
    ...
    if (isWinningDualLeg && channelKey !== 'SINGLE') {
      ...
      if (this.config.antiMartEnabled) {
        ...
        this.sendLog(`✅ [${channelKey}] WIN $${profit.toFixed(2)} — Anti-Mart stake $${channel.stake.toFixed(2)} (step ${channel.step})`);
      } else if (this.config.recoveryEnabled !== false) {
        if (this._shouldResetMartingaleOnWin()) {
    ...
    if (isDualStrategy && channelKey !== 'SINGLE') {
      ...
      if (switchThreshold > 0 && !won && channel.consecutiveLosses >= switchThreshold) {
        if (this.config.autoSwitchMarkets !== false && !this._isWinningDualStrategy()) {
        ...
        if (this.strategy === 'OU_WINNING' || this.strategy === 'EO_WINNING') {
          ...
          if (lastTwo.length === 2) {
            ...
            if (bothLost) {
              ...
              if (this.config.autoSwitchMarkets !== false) {
                this._maybeRotateMarketForLeg(priorityDir, 'both legs lost')
                  || this._migrateWinningDualToBalancedMarket();
              }
            } else if (net > 0) {
              this.dualNetLossStreak = Math.max(0, (this.dualNetLossStreak || 0) - 1);
            } else if (loser) {
            ...
            const switchThreshold = this._getSwitchAfterLossesThreshold();
            if (this.config.autoSwitchMarkets !== false) {
              const hotLeg = dirs.find(d => (this.channels[d]?.consecutiveLosses || 0) >= switchThreshold);
              if (hotLeg) {
                this.sendLog(
                  `🔄 [${hotLeg}] ${this.channels[hotLeg].consecutiveLosses} consec losses · migration scan…`
                );
                this._maybeRotateMarketForLeg(hotLeg, 'hot leg')
                  || this._migrateWinningDualToBalancedMarket();
              }
            }
    ...
    } else if (!['MATCHES', 'BOTH', 'BOTH5', 'OMNISNIPER'].includes(this.strategy) && !won) {
      const switchAfter = Number(this.config.switchAfterLosses) || 0;
      if (switchAfter > 0 && this.channels[channelKey]?.consecutiveLosses >= switchAfter) {
        this._switchMarketLegacy(direction);
      }
    }
  ...
  _executeLegacyCycle() {
    const isDual = this.strategy === 'OU_WINNING' || this.strategy === 'EO_WINNING' || this.strategy === 'BOTH' || this.strategy === 'BOTH5';
    ...
    if (isDual) {
      ...
      if (false) {
        this.updateStatus('🛡️ Dual hedge idle — arms after 2 losses or a 2-loss leg streak', true);
        this._scheduleNext(OMNI_SCAN_MS);
        return;
      }
